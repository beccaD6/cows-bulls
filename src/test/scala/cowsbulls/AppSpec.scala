...
class AppSpec extends Specification {
    ...
    private def runApp(guess: String): IO[List[String]] = for {
      ...
      _ <- new App(createPrintlnFn(outputRef), IO.pure(guess)).run()
...
//    app.run().map(_=>output)
  ...
  "tell the user when they give invalid input" in {
    runApp("12ee").unsafeRunSync() must throwA( new RuntimeException("Please provide a valid number"))
  }

  "tell the user when they give an incorrect amount of digits" in {
    runApp("23").unsafeRunSync() must throwA( new RuntimeException("Please provide a 4 digit number"))
  }
