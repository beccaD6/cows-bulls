...
class AppSpec extends Specification {
  ...
  private def runApp(guesses: List[String]): IO[List[String]] = for {
    ...
    res <- new App(createPrintlnFn(outputRef), createReadlnFn(inputRef)).run()
  ...
  "App" should {
    ...
    "give errors for incorrect input then prompt user to guess again" in {
      ...
      output must contain("Make a guess","bulls: 3, cows: 1", "Please provide a valid number", "Please provide a 4 digit number")
    }
  }
